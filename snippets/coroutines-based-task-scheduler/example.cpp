...
template <typename T> struct PromiseResult {
...
template <> struct PromiseResult<void> {
...
public:
  struct promise_type : detail::PromiseResult<T> {
    ...
    template <typename Awaitable> auto await_transform(Awaitable &&awaitable) {
...
private:
  ...
  void WorkerLoop(WorkerThread *worker, size_t worker_id) {
    while (!shutdown_.load()) {
      ...
      if (!found_task) {
        ...
        global_condition_.wait_for(lock, std::chrono::milliseconds(10), [this] {
          return !global_queue_.empty() || shutdown_.load();
        });
  ...
  bool TryStealWork(size_t current_worker_id,
                    std::coroutine_handle<> &stolen_task) {
    for (size_t i = 1; i < workers_.size(); ++i) {
      size_t target_id = (current_worker_id + i) % workers_.size();
      WorkerThread *target_worker = workers_[target_id].get();

      std::unique_lock<std::mutex> lock(target_worker->queue_mutex_,
                                        std::try_to_lock);
...
template <typename F> struct ThreadPoolAwaitable {
  F function_;
...
template <typename F> auto RunOnThreadPool(F &&func) {
...
coro_scheduler::Task<void> LogProgress(std::vector<std::string> &log,
                                       std::mutex &log_mutex) {
...
int main() {
  ...
  while (!answer.IsReady())
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  std::cout << "compute_answer() = " << answer.Get()
            << " (expected 42): " << (answer.Get() == 42 ? "PASS" : "FAIL")
            << "\n";
  ...
  while (!progress.IsReady())
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  progress.Get();
  std::cout << "log_progress() recorded " << log.size()
            << " steps: " << (log.size() == 3 ? "PASS" : "FAIL") << "\n";

  auto failing = FailingTask();
  while (!failing.IsReady())
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  bool caught = false;
  try {
    failing.Get();
