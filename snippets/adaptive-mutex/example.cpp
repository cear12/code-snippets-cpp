...
class AdaptiveMutex {
private:
    enum class State : uint32_t { kUnlocked = 0, kLocked = 1, kLockedWithWaiters = 2 };
    ...
    bool TrySpinLock() {
        ...
        return state_.compare_exchange_weak(expected, static_cast<uint32_t>(State::kLocked),
...
public:
    ...
    void Lock() {
        ...
        while (true) {
            uint32_t expected = static_cast<uint32_t>(State::kUnlocked);
            if (state_.compare_exchange_weak(expected, static_cast<uint32_t>(State::kLocked),
                                              std::memory_order_acquire, std::memory_order_relaxed)) {
                auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
                                     std::chrono::high_resolution_clock::now() - start_time)
                                     .count();
            ...
            if (expected == static_cast<uint32_t>(State::kLocked)) {
                state_.compare_exchange_weak(expected, static_cast<uint32_t>(State::kLockedWithWaiters),
    ...
    void Unlock() {
        ...
        if (state_.compare_exchange_strong(expected, static_cast<uint32_t>(State::kUnlocked),
