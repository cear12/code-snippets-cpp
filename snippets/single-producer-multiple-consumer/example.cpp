...
template <typename T, size_t Capacity> class SPMCQueue {
private:
...
public:
  ...
  template <typename U> void Enqueue(U &&item) {
  ...
  bool Empty() const {
    return producer_pos_.load(std::memory_order_relaxed) ==
           consumer_cursor_.load(std::memory_order_relaxed);
  }

  bool Full() const {
    return (producer_pos_.load(std::memory_order_relaxed) -
            consumer_cursor_.load(std::memory_order_relaxed)) >= Capacity;
  }
...
template <typename T, size_t Capacity = 1024> class ManagedSPMCSystem {
private:
...
public:
  ...
  ManagedSPMCSystem(BatchProcessor &&batch_processor, size_t num_consumers,
                    bool /* batch_tag */)
  ...
  template <typename U> bool TryProduce(U &&item) {
    return queue_.TryEnqueue(std::forward<U>(item));
  }

  template <typename U> void Produce(U &&item) {
  ...
  void StartConsumers(size_t num_consumers, bool use_batching) {
    ...
    for (size_t i = 0; i < num_consumers; ++i) {
      consumers_.emplace_back(
          [this, i, use_batching] { ConsumerLoop(i, use_batching); });
    }
  }
};
...
int main() {
  ...
  // consumer threads drains them via a processor callback.
  {
    ...
    ManagedSPMCSystem<int, 1024> system(
        [&processed](int) {
    ...
    std::cout << "[ManagedSPMCSystem] processed " << processed.load() << "/"
              << kItems << ": "
